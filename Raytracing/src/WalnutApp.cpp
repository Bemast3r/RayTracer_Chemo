...
public:
	...
	void Render() {
		...
		// oder das Bild von der Größe oder  der Höhe verändert
		//  mache folgendens: 
		if (!m_Image || m_ViewportWidth !=m_Image->GetWidth() || m_ViewportHeight != m_Image->GetHeight()) {
			m_Image = std::make_shared<Image>(m_ViewportWidth, m_ViewportHeight, ImageFormat::RGBA); 
			delete[] m_ImageData; // ImageBuffer löschen 
			// ImageBuffer neu belegen
			// Mal 4 ist unnötig (da RGBA 4 bite benötigt) da uint32_t 4 Bytes groß ist 
			m_ImageData = new uint32_t[m_ViewportWidth * m_ViewportHeight];
		}

		for (uint32_t i = 0; i < m_ViewportWidth * m_ViewportHeight; i++){
			m_ImageData[i] =Random::UInt(); // Magenta Pixel werden eingeführt (RBGA)
